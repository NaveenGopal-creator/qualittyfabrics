...
// ═══════════════════════════════════════════════════
const GEMINI_API_KEY = 'AQ.Ab8RN6KA4kKJpCPzQ4uqqaVDAIDJ-Ecul-DokgRtglisUo__sQ';

// Builds fresh URL each call so key is always current
...
function getGeminiURL() {
  const model = QF_MODELS[qfModelIdx] || QF_MODELS[0];
...
async function qfCallGemini(contents) {
  // Try every model in the fallback list until one works
  for (let i = 0; i < QF_MODELS.length; i++) {
    qfModelIdx = i;
    const url   = getGeminiURL();
    const model = QF_MODELS[i];
    console.log('[QF] Trying model:', model);
    ...
    console.log('[QF] Model', model, '→ HTTP', res.status);

    // Rate limited — wait 65s full window then retry automatically
    if (res.status === 429) {
      console.warn('[QF] 429 on', model, '— clearing log and waiting 65s');
      qfReqLog.length = 0;  // clear request history
      qfRemoveTyping();
      qfShowCountdown(65000);
      await new Promise(r => setTimeout(r, 65000));
      qfShowTyping();
      // Retry this same model once immediately after wait
      try {
        ...
        if (r2.ok && !d2.error) {
          ...
          if (t2) { console.log('[QF] Success after rate-limit wait on', model); return { reply: t2 }; }
        }
        console.warn('[QF] Still failing after wait — trying next model');
      } catch(e2) { console.warn('[QF] Retry error:', e2.message); }
    ...
    if (res.status === 404 || (data.error && (
      ...
      console.warn('[QF] Model', model, 'not found — trying next');
      continue;
    }
    ...
    if (!res.ok || data.error) {
      console.error('[QF] Error from', model, ':', JSON.stringify(data));
    ...
    if (!text) {
      console.error('[QF] Empty candidates from', model, ':', JSON.stringify(data));
      throw new Error('Empty response');
    }
    console.log('[QF] Success with model:', model);
    return { reply: text };
  }
...
async function qfSend() {
  ...
  if (!GEMINI_API_KEY || GEMINI_API_KEY === 'YOUR_GOOGLE_GEMINI_API_KEY') {
    qfAddMessage('bot', '⚠️ API key not set. Open <b>shared.js</b> line 6 and paste your key from <a href="https://aistudio.google.com/app/apikey" target="_blank" style="color:#c9a84c">AI Studio</a>.');
    return;
  }
